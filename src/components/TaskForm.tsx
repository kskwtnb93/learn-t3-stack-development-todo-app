...
export const TaskForm = () => {
  const { createTaskMutation, updateTaskMutation } = useMutateTask(); // カスタムフック
  const { editedTask } = useStore(); // Zustandのステート
  const update = useStore((state) => state.updateEditedTask); // Zustandのステート更新用関数
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-5 text-center">
      {(updateTaskMutation.isLoading || createTaskMutation.isLoading) && (
        <p className="mb-2 text-green-500">Mutation undet process...</p>
      )}
      <input
        type="text"
